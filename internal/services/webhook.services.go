...
func (s *OmieService) ProcessarWebhookBaixaRealizada(data models.WebhookBaixaRealizadaResponse) (int, error) {
	fmt.Printf("Processando webhook de baixa realizada: Código Lançamento Omie: %d, Código Cliente: %d, Data: %s, Data Crédito: %s, Valor: %.2f\n",
		data.CodigoLancamentoOmie, data.CodigoCliente, data.Data, data.DataCred, data.Valor)
	result, err := repositories.WebhookUpdateConferido(database.ConnectToDB(), data.CodigoLancamentoOmie, data.Data, data.DataCred, data.Observacao, data.Valor)
	if err != nil {
